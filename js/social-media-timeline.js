...
Timeline.prototype.updateVis = function() {
    ...
    circle.on("mouseover", function(d) {
        ...
        d3.selectAll(".circle").transition().duration(376).attr("opacity", 0.1);
        d3.select(this).transition().duration(376).attr("opacity", 0.7)});

    circle.on("mouseout", function(d) {
        vis.title.text("Social networks");
        vis.platformInfo.innerHTML = "";
        d3.selectAll(".circle").transition().duration(376).attr("opacity",.5);
    });
